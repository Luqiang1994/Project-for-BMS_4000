...
namespace BMSSoftware.Devices
{
    public class zlgcan
    {
        ...
        public class Method
        {
            ...
            public static float GetMotorolaSignalValue(int start, uint lenth, byte[] srcData, float factor = 1, float offset = 0, int initialValue = 0, float min = 0, float max = 65535)
            {
                long[] VAL = new long[lenth];
                int mByteNum = 0;
                int mByteBit = 0;
                mByteNum = start / 8;
                mByteBit = start % 8;
                ...
                {
                    VAL[i] = (srcData[mByteNum] & (0x1 << mByteBit)) >> mByteBit;
                    if (mByteBit >= 7)
                    {
                        mByteNum--;
                        mByteBit = 0;
                ...
                destVal = destVal * Convert.ToDecimal(factor) + Convert.ToDecimal(offset);
                if (destVal > Convert.ToDecimal(max))
                {
                    destVal = Convert.ToDecimal(max);
                }
                if (destVal < Convert.ToDecimal(min))
                {
                    destVal = Convert.ToDecimal(min);
                }

                return Convert.ToSingle(destVal);
            }

            public static float getByteVal(int start, uint lenth, byte[] srcData, float factor = 1, int offset = 0, int initialValue = 0, int min = 0, float max = 65535)
            {
                long[] VAL = new long[lenth];
                int mByteNum = 0;
                int mByteBit = 0;
                ...
                destVal = destVal * Convert.ToDecimal(factor) + offset;
                if (destVal > Convert.ToDecimal(max))
                {
                    destVal = Convert.ToDecimal(max);
                }
